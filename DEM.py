        ...
        for angle in range(360):
            ...
            for step in range(1, max_steps):
                ...
                if 0 <= target_row < dem_matrix.shape[0] and 0 <= target_col < dem_matrix.shape[1]:
                    altitude_terreny = dem_matrix[target_row, target_col]
                    
                    if altitude_terreny == src.nodata or altitude_terreny < -500:
                        continue
                        
                    distancia_m = step * pixel_size_m
                    theta = np.degrees(np.arctan2((altitude_terreny - user_alt), distancia_m))
...
def get_user_altitudee_from_dem(dem_path, lat, lon):

    with rasterio.open(dem_path) as src:
        transformer = Transformer.from_crs("EPSG:4326", src.crs, always_xy=True)
        x_crs, y_crs = transformer.transform(lon, lat)
        
        projected_coords = [(x_crs, y_crs)]
        generator = src.sample(projected_coords)
        
        for pixel_values in generator:
            altitude = pixel_values[0]
            return altitude


def download_dynamic_dem(lat, lon, api_key, radius_km=35, dem_type="COP90"):
...
def get_visible_peaks(observer_lat, observer_lon, angle_start, fov, radius_km=25):
    ...
    try:
        ...
        print(f"Error connecting with OpenStreetMap: {e}")
        return [], 0, []

    elements = data.get('elements', [])
    all_peaks_zone = len(elements)
    ...
    for element in elements:
        if 'tags' in element and 'name' in element['tags']:
            ...
            all_peaks.append({
                ...
                'In the Photo?': '✅ Yes' if in_view else '❌ No'
            })

    # order by elevation
    all_peaks.sort(key=lambda x: x['Elevation (m)'], reverse=True)
    
    visible_peaks = [cim for cim in all_peaks if cim['In the Photo?'] == '✅ Yes']
    return visible_peaks, all_peaks_zone, all_peaks